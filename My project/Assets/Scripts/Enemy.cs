...
public class Enemy : MonoBehaviour
{
    ...
    {
        if (direction > 0 && transform.localScale.x < 0) // Player อยู่ทางขวา แต่ enemy หันซ้าย
        {
            Vector3 ls = transform.localScale;
            ls.x *= -1;  // พลิกกลับ
            transform.localScale = ls;
        }
        else if (direction < 0 && transform.localScale.x > 0) // Player อยู่ทางซ้าย แต่ enemy หันขวา
        {
            Vector3 ls = transform.localScale;
            ls.x *= -1;
            transform.localScale = ls;
